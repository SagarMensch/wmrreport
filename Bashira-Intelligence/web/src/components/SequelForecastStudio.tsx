...
// CURATED PALETTE â€” Premium Light Institutional Aesthetic
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
const C = {
  bg: '#F8F6F3',
  surface: '#FFFFFF',
...
export default function SequelForecastStudio({ onClose }: { onClose: () => void }) {
  ...
  // â”€â”€ Load anomalies + model info on mount â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  useEffect(() => {
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col" style={{ background: C.bg, fontFamily: '"Figtree", sans-serif' }}>
            ...
            <p className="text-[10px] uppercase font-semibold tracking-[0.15em] mt-0.5" style={{ color: C.t3 }}>
              ...
              {modelInfo && (
                <span style={{ color: modelInfo.autogluon_loaded ? C.safe : C.warn, marginLeft: '8px' }}>
                  Model state: {modelInfo.autogluon_loaded ? 'Live' : 'Cached'}
                </span>
              )}
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto w-full max-w-[1600px] mx-auto px-8 py-8 grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        {activeTab === 'forecast' ? (
          <>
            {/* â”€â”€ LEFT COLUMN: CONTROL PANELS â”€â”€ */}
            <div className="md:col-span-8 flex flex-col gap-6">
              ...
              <div className="p-8 rounded-xl" style={{ background: C.surface, border: `1px solid #E5E5E5`, boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                ...
                <p className="text-[12px] mb-6" style={{ color: C.t3 }}>Real ML prediction from the live predictive ensemble and completion engine.</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                      ...
                      {targetResult.predicted_progress_4w !== undefined && (
                        <MetricCard
                          ...
                          sub={`Î” ${(targetResult.predicted_delta_4w ?? 0) > 0 ? '+' : ''}${targetResult.predicted_delta_4w}pp`}
                          color={C.safe}
                        />
                      )}
                    </div>
                      ...
                      <div className="p-4 rounded-lg mb-6" style={{ background: '#F0FDF4', border: '1px solid #BBF7D0' }}>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div>
                            ...
                            <div style={{ fontSize: '13px', fontWeight: 700, color: C.t1 }}>{targetResult.survival.completion_date_early || 'â€”'}</div>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: '13px', fontWeight: 700, color: C.t1 }}>{targetResult.survival.completion_date_late || 'â€”'}</div>
                          </div>
                        </div>
                      </div>
                    )}
              ...
              <div className="p-8 rounded-xl" style={{ background: C.surface, border: '1px solid #E5E5E5', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                ...
                <p className="text-[12px] mb-6" style={{ color: C.t3 }}>Batch predict all active wells using the live predictive engine and detect tier changes or anomalies.</p>
                <div className="flex items-center gap-4">
                  <button
              ...
              <div className="p-8 rounded-xl" style={{ background: C.surface, border: '1px solid #E5E5E5', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                ...
                <p className="text-[12px] mb-6" style={{ color: C.t3 }}>Full pipeline: export data, run remote GPU retraining, refresh the ensemble, and rebuild driver outputs.</p>
                <div className="flex items-center gap-4">
                  <button
                ...
                <div className="p-6 rounded-xl" style={{ background: C.surface, border: '1px solid #E5E5E5' }}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[12px]">
                    ...
                    {modelInfo.ag_best_model && (
                      <div><span style={{ color: C.t3 }}>Primary:</span> <span style={{ fontWeight: 600 }}>Live ensemble</span></div>
                    )}
                    ...
                    {modelInfo.training_metrics && (
                      <>
                        ...
                        <div><span style={{ color: C.t3 }}>RÂ²:</span> <span style={{ fontWeight: 600 }}>{modelInfo.training_metrics.r2?.toFixed(4)}</span></div>
            ...
            {portfolio ? (
              <>
                ...
                {portfolio.risk_drivers && portfolio.risk_drivers.length > 0 && (
                  <div className="p-8 rounded-xl" style={{ background: C.surface, border: '1px solid #E5E5E5' }}>
                    <h2 className="text-[11px] font-bold tracking-[0.1em] uppercase mb-4" style={{ color: C.t1 }}>Global Risk Drivers</h2>
                    <div className="space-y-2">
        ...
        {/* â”€â”€ RIGHT COLUMN: ANOMALY FEED â”€â”€ */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-baseline gap-2 mb-2">
                        ...
                        <span className="text-[10px]" style={{ color: C.t4 }}>â†’</span>
                        <TierBadge tier={an.new_tier} />
                      </div>
                      <div className="flex items-center justify-between text-[10px]">
                        <span style={{ color: c }}>{an.severity} {isEscalation ? 'ESCALATION' : 'RECOVERY'}</span>
                        <span style={{ color: C.t2 }}>Î” {an.delta > 0 ? '+' : ''}{an.delta.toFixed(1)}</span>
                      </div>
                    </div>
                  );
                })
