...
@lru_cache(maxsize=1)
def _resolve_predict_tables() -> Dict[str, str]:
    """
    Resolve the local snapshot/history tables for predictive queries.

    The client environment historically used aliases like `WMR`, but local
    offline clones only expose the physical staging tables.
    """
    conn = _get_sql_conn()
    if conn is None:
        return {
            "snapshot": "WellMonitoringReport",
            "history": "WMR_Full",
        }
    ...
    return {
        "snapshot": pick("WellMonitoringReport", "WellMonitoringReport_Latest", "WMR_Full"),
        "history": pick("WMR_Full", "WellMonitoringReport", "WellMonitoringReport_Latest"),
    }


def _fetch_well_from_sql(conn, well_name: str) -> Optional[Dict]:
    """Fetch a single well's latest data from WellMonitoringReport."""
    tables = _resolve_predict_tables()
    snapshot_table = tables["snapshot"]
    cursor = conn.cursor()
    cursor.execute(
        f"SELECT * FROM [{snapshot_table}] WHERE well_name_after_spud = ?",
        (well_name,)
    )
...
def _fetch_well_history(conn, well_name: str) -> List[Dict]:
    ...
    tables = _resolve_predict_tables()
    history_table = tables["history"]
    cursor = conn.cursor()
    cursor.execute(
        f"""SELECT well_name_after_spud, over_all_progress_percentages, Week_Number
           FROM [{history_table}]
