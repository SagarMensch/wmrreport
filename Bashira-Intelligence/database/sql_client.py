...
class SQLClient:
    ...
    def _fix_well_counting(self, sql: str) -> str:
        """Auto-fix only explicit well-name counts to use pdo_well_id."""
        import re

        if 'pdo_well_id' in sql.lower():
            return sql

        sql = re.sub(
            r'COUNT\(\s*DISTINCT\s*\[?well_name_after_spud\]?\s*\)',
        ...
        sql = re.sub(
            r'COUNT\(\s*DISTINCT\s*\[?well_name\]?\s*\)',
            'COUNT(DISTINCT [pdo_well_id])',
            sql,
            flags=re.IGNORECASE
        )
